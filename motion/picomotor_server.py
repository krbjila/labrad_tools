...
class PicomotorServer(DeviceServer):
    ...
    @inlineCallbacks
    def call_if_available(self, method, *args):
        try:
    ...
    def get_position(self, c, axis):
        ...
        position = yield self.call_if_available(dev.get_position, axis)
    ...
    def move_rel(self, c, axis, distance):
        dev = self._select_device(c)
        yield self.call_if_available(dev.move_rel, axis, distance)

    @inlineCallbacks
    @setting(12, 'abort')
    def abort(self, c):
        dev = self._select_device(c)
        yield self.call_if_available(dev.abort)

    @inlineCallbacks
    @setting(13, 'stop', axis='i')
    def stop(self, c, axis):
        dev = self._select_device(c)
        yield self.call_if_available(dev.stop, axis)

    @inlineCallbacks
    @setting(14, 'set home', axis='i', position='i')
    def set_home(self, c, axis, position=0):
        dev = self._select_device(c)
        yield self.call_if_available(dev.set_home, axis, position)

    @inlineCallbacks
    @setting(15, 'motor check')
    def motor_check(self, c):
        dev = self._select_device(c)
        yield self.call_if_available(dev.motor_check)

    @inlineCallbacks
    @setting(16, 'save settings')
    def save_settings(self, c):
        dev = self._select_device(c)
        yield self.call_if_available(dev.save_settings)

    @inlineCallbacks
    @setting(17, 'query motor type', axis='i', returns='i')
    def query_motor_type(self, c, axis):
        dev = self._select_device(c)
        type = yield self.call_if_available(dev.query_motor_type, axis)
        returnValue(type)
    ...
    def move_abs(self, c, axis, position):
        dev = self._select_device(c)
        yield self.call_if_available(dev.move_abs, axis, position)

    @inlineCallbacks
    @setting(19, 'motion done', axis='i', returns='b')
    def motion_done(self, c, axis):
        dev = self._select_device(c)
        done = yield self.call_if_available(dev.motion_done, axis)
        returnValue(done)
    ...
    def set_velocity(self, c, axis, speed):
        dev = self._select_device(c)
        yield self.call_if_available(dev.set_velocity, axis, speed)

    @inlineCallbacks
    @setting(21, 'get velocity', axis='i', returns='i')
    def get_velocity(self, c, axis):
        dev = self._select_device(c)
        speed = yield self.call_if_available(dev.get_velocity, axis)
        returnValue(speed)
    ...
    def reset(self, c):
        dev = self._select_device(c)
        yield self.call_if_available(dev.reset)
