...
..
    ...
    name = picomotor
    version = 1
    description = server for Newport picomotors
...
from server_tools.device_server import (
    DeviceServer,
    DeviceWrapper,
    get_connection_wrapper,
    get_device_wrapper,
)
...
class PicomotorServer(DeviceServer):
    ...
    def _select_device(self, c):
        try:
            ...
            raise Exception(
                "Please select a device first; devices: {}".format(self.devices.keys())
            )
