...
from lib.hv_ramps import hvRampMaker

VREFP = 7
VREFN = -7

VOLTAGE_RANGE = (VREFN, VREFP)
DAC_BITS = 20
...
class hvChannel(object):
    """ wrapper for single hv stable dac analog channel 

    example_config = {
        'loc': 0, # in range(6)
        'name': 'DACA0', # unique string 
        'mode': 'auto', # 'auto' or 'manual'
        'manual_output': 0, # default manual voltage. between (-5,5).
    }
    """
    def __init__(self, config):
        """ defaults """
        self.channel_type = 'hv'
        self.mode = 'auto'
        self.manual_output = 'manual'
        board_name = config['board_name']
        loc = config['loc']
        self.name = 'DAC'+board_name+str(loc).zfill(2)
        self.voltage_range = [-2., 2.]
...
#        self.bitfile = 'dac.bit'
        self.mode_ints = {'idle': 0, 'load': 1, 'init': 2, 'run': 3}
        self.mode_wire = 0x00
        self.sequence_pipe = 0x80
        self.channel_mode_wire = 0x09
        self.manual_voltage_wires = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06]

        # Max clock rate on this dacs is 25 MHz
        # Max throughput is then 25 MHz / (24 data bits + 2*2 cycles for sync held high)
        self.clk = 25e6 / (24 + 2*2)
        self.mode = 'idle'
    ...
    def make_sequence_bytes(self, sequence):
        """ 
        take readable {channel: [{}]} to programmable [ramp_rate[20], duration[28]]
        """
        ...
        unsorted_ramps = []
        for c in self.channels:
            ramps = hvRampMaker(sequence[c.key]).get_programmable()
            T = 0
            for r in ramps:
        ...
        # modified account for 20 bit output of ramp_rate
        for r in sorted_ramps:
            byte_array += [int(eval(hex(ramp_rate(r[2]['dv'], r[2]['dt']))) 
                       >> i & 0xff) for i in range(0, 24, 8)]
