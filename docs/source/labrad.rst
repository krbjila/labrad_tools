...
Labrad ``Signals`` provide a way to reverse this control flow, by letting the server send data to the client (and trigger callbacks that use this data). See the `Labrad signals source code <https://github.com/labrad/pylabrad/blob/master/labrad/server.py>`__ for more details.
