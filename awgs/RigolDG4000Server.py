...
..
    ...
    instancename = %LABRADNODE%_dg4000
...
class DG4000Server(LabradServer):
    """Provides access to Rigol DG800 series AWGs."""

    name = "%LABRADNODE%_dg4000"
    ...
    def get_devices(self, c):
        ...
        Lists connected DG4000 AWGs. Note that the function connects to each device to check its ID.

        Args:
            c: A LabRAD context (which is passed on to passed to :meth:`select_device`)

        Yields:
            A list of strings corresponding to the IDs of connected DG4000 AWGs
        """
    ...
    def select_device(self, c, device):
        ...
        Select a connected DG4000 AWG

        Args:
            c: A LabRAD context (not used)
            device (string): The ID of the DG4000 AWG to connect to, as returned by :meth:`get_devices`
        """
    ...
    def set_sin(self, c, channel, freq, amplitude, offset, phase):
        ...
        yield self.USB.write(
            ":SOUR%d:APPL:SIN %f,%f,%f,%f"
            % (channel, min(200e6, max(1e-6, freq)), amplitude, offset, phase)
        )
    ...
    def set_impedance(self, c, channel, impedance=50, inf=False, low=False):
        ...
        if inf:
            ...
            yield self.USB.write(
                ":OUTP%d:IMP %d" % (channel, min(10000, max(1, impedance)))
            )
    ...
    def set_ncycles(self, c, channel, ncycles):
        ...
        yield self.USB.write(
            ":SOUR%d:BURS:NCYC %d" % (channel, min(500000, max(1, ncycles)))
        )
