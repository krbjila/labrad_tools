...
class Valon5009Server(LabradServer):
        ...
        for i in interfaces:
            ...
            try:
                ...
                if "Illegal command!" in id1:
                    next_line = yield self.USB.query('*IDN?')
                    next_next_line = yield self.USB.query('*IDN?')
    ...
    def set_channel(self, c, channel):
        """Selects whether channel 1 or 2 is used.

        Args:
            c: The LabRAD context (not used)
            channel (boolean): False for 1, True for 2
        """
        yield self.USB.query("SOUR {}".format(2 if channel else 1))
    ...
    def set_attenuation(self, c, attenuation):
        ...
        return float(re.findall(r'\d*\.?\d+', reply)[0])
    ...
    def set_freq(self, c, freq):
        ...
        reply = yield self.USB.read()
        return float(re.findall(r'\d*\.?\d+', reply)[1])
