...
class SynthesizerServer(LabradServer):
    ...
    @staticmethod
    def compile_timestamp(channel, address, timestamp, phase_update, ptw, atw, ftw):
        """
        ...
        buffer = b''
        buffer += struct.pack('>B', 0xA1) # FIFO transaction
        buffer += struct.pack('>B', channel) # Channel
        buffer += struct.pack('>H', address) # Address
        buffer += struct.pack('>I', timestamp) # Timestamp
        buffer += struct.pack('>H', phase) # Phase
        buffer += struct.pack('>H', atw) # Amplitude
        buffer += struct.pack('>I', ftw) # Frequency
        buffer += struct.pack('>B', 0x00) # Terminator
    ...
    def _write_timestamps(self, timestamps, channel):
        ...
        for i, s in enumerate(timestamps):
            ...
            buffer += SynthesizerServer.compile_timestamp(channel, address, timestamp, phase_update, ptw, atw, ftw)
        self.ser.write(buffer)
        self.ser.flush()

    @setting(5, timestamps='s', channel='i')
    def write_timestamps(self, c, timestamps, channel):
        """
        ...
        Args:
            ...
            channel (int): An integer between 0 and 3 determining the channel to program
        """
        self._write_timestamps(loads(timestamps), channel)
