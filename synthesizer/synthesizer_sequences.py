...
def validate_parameters(
    ...
    if duration is not None and (duration < 0 or duration > MAX_DURATION):
        raise ValueError(
            "Duration {} must be between zero and {}.".format(duration, MAX_DURATION)
        )
    ...
    if frequency is not None and (frequency < 0 or frequency > MAX_FREQUENCY):
        raise ValueError(
            "Frequency {} must be between 0 and {}.".format(frequency, MAX_FREQUENCY)
        )
...
class Timestamp(RFBlock):
    ...
    def __init__(
        ...
        phase: Optional[float] = None,
        frequency: Optional[float] = None,
        wait_for_trigger: Optional[bool] = False,
        digital_out: Optional[dict] = {},
        absolute_phase: Optional[bool] = False,
    ) -> None:
        """
        Args:
...
class PhaseRamp(RFBlock):
    ...
    def compile(self, state: SequenceState) -> List[RFBlock]:
        validate_parameters(
            self.duration, self.amplitude, self.start_phase, self.frequency
        )
...
class Transition:
    ...
    def __init__(
        ...
        if (
            ...
            or frequency - frequency_offset > MAX_FREQUENCY
        ):
            raise ValueError(
...
def DROID_C3PO(wait: float, pulse: RFPulse = None):
    if pulse is None:
        pulse = PiOver2Pulse()
            ...
            if duration > MAX_DURATION:
                raise ValueError(
                    "The duration {} s of channel {}'s sequence exceeds the maximum duration of {} s after {}".format(
                        duration, channel, MAX_DURATION, block
                    )
                )
        durations.append(duration)
        ...
        if len(compiled_channel) > MAX_LENGTH:
            raise ValueError(
                "The length {} of channel {}'s sequence exceeds the maximum length of {}".format(
                    len(compiled_channel), channel, MAX_LENGTH
                )
            )
...
def plot_sequence(seq: List[RFBlock]):
    ...
    fig = make_subplots(
        ...
        row_heights=[0.25, 0.25, 0.25] + [0.2 / N_DIGITAL] * N_DIGITAL,
    )
...
if __name__ == "__main__":
    droid_c3p0 = DROID_C3PO(1e-3)
    # seq = {
    #     "0": [
    #         SetTransition(Transition(1e6, [1], [100e3])),
    #         droid_c3p0[0]
    #         ],
    #     "1": [
    #         SetTransition(Transition(2e6, [1], [100e3])),
    #         droid_c3p0[1]
    #         ],
    # }
    # plot_sequence(seq)

    from pulse_sequences import display_pulses

    # display_pulses(DROID60(1e-3))
    display_pulses(droid_c3p0[0])
    # display_pulses(droid_c3p0[1])
